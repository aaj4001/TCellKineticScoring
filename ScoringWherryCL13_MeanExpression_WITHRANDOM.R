...
cl13_RandomScores = data.frame(cl13_RandomScores)
cl13_RandomScores$State = rep(names(DEG_SignaturesConverted),each = nperm)
cl13_RandomScores = reshape2::melt(cl13_RandomScores)
cl13_RandomScores$SigType = "Baseline"

# ggdensity(cl13_RandomScores,"value",color ="variable",facet.by = "State")
# ggline(cl13_RandomScores,"variable","value",facet.by = "State",add = "mean_se")

## Does Signature Scoring in Wherry Armstrong
cl13Scores = matrix(nrow = length(DEG_SignaturesConverted),ncol = ncol(Arm))

for(i in 1:length(DEG_SignaturesConverted)) cl13Scores[i,] = SignatureScore_UpMinusDown(cl13,DEG_SignaturesConverted[[i]])
cl13Scores = data.frame(cl13Scores)
colnames(cl13Scores) = colnames(cl13)
cl13Scores$State = names(DEG_SignaturesConverted)
cl13Scores = reshape2::melt(cl13Scores)
cl13Scores$SigType = "Exp"

# Combines Scores, plots, does stats
CombinedScores = rbind(cl13_RandomScores,cl13Scores)
...
saveRDS(ggarrange(Plots[[1]],Plots[[2]],Plots[[3]],CombinedPlot,nrow = 4),"Wherry_Cl13.rds")
